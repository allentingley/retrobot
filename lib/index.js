...
let sprintID = 'No Sprint ID Defined';
...
function captureRetrospective(message) {
  ...
  if (type === `plus`) { 
    return reply(message, `I've added \`${line}\` to the "worked well" list for the retrospective.`);
  }
  if (type === `minus`) { 
    return reply(message, `I've added \`${line}\` to the "needs improvement" list for the retrospective.`);
  }
...
function command(message) {
  let [me, cmd, sprintID, timelimit] = message.text.split(' ');
  ...
  if (aliases[cmd] === start) {
    ...
    const autoend = timelimit;
    let msg = '🔔 Retrospective for Sprint '+  sprintID +' is now recording';
        ...
        slack.dnd.info({ token, user }, (error, dnd) => {
          ...
          slack.users.getPresence({ token, user }, (error, presence) => {
            ...
            slack.im.open({ token, user }, (error, data) => {
              ...
              reply({ channel: dms[user] }, '👂 Hi! I\'m Retrobot. I\'m here to collect information about the most recent sprint at Acrobatiq for our retrospective.' +
                'A retrospective is structured around "things which worked well" and "things which need improvement".'+
                '\nTell me about your experience this sprint. I will collect the team\'s thoughts, and then we will vote as a group to determine adjustments we can make.' +
                '\nStart lines with `+` for "worked well" and `-` for "needs ' +
                'improvement".'+
                '\nFor example: "+ retrobot is good at his job!" or "- retrobot is annoying".');
            });
          });
