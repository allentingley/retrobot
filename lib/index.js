        ...
        slack.dnd.info({ token, user }, (error, dnd) => {
          ...
          slack.users.getPresence({ token, user }, (error, presence) => {
            ...
            slack.im.open({ token, user }, (error, data) => {
              ...
              reply({ channel: dms[user] }, '👂 Tell me about your retrospective for the ' + SprintID + ' sprint,'
...
function stopRetro(message) {
  ...
  inRetro = false;
  reply(message, 'The retrospective for sprint ' + SprintID + ' has now finished, give me a second or ' +
