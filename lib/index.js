        ...
        slack.dnd.info({ token, user }, (error, dnd) => {
          ...
          slack.users.getPresence({ token, user }, (error, presence) => {
            ...
            slack.im.open({ token, user }, (error, data) => {
              ...
              reply({ channel: dms[user] }, 'Hi! I\'m retrobot. I collect information from the team for our Sprint retrospectives.' +
                '\n👂 Tell me about your experience for the latest sprint, structured around things which worked or things which need improvement.'
                '\nStart lines with `+` for "worked well" and `-` for "needs ' +
                'improvement".'+
                '\n For example: "+ retrobot did a great job collecting retrospective information"');
            });
          });
...
function stopRetro(message) {
  ...
  inRetro = false;
  reply(message, 'The retrospective for the sprint has now finished, give me a second or ' +
